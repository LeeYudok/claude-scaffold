...
presets/                  preset fragments (copy-overwrite model)
  ...
  forge-gitlab/           GitLab forge — glab, MRs, GitLab 19 manual close
...
| `rust` | rust.md (paths: **/*.rs, Cargo.toml) | `cargo check` + `clippy` |
...
execution), it downloads the `CLAUDE_SCAFFOLD_REPO` tarball (default: the GitHub
repo; owner is a placeholder until finalized) into a temporary directory and
